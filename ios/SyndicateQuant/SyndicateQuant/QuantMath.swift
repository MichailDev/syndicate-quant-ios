...
enum QuantMath {
  ...
  static func probabilityInterval(_ p: Double, sample: Int, dcs: Double, marketMAD: Double) -> (
  ...
  static func dixonColes(_ lh0: Double, _ la0: Double, rho: Double = -0.055, maxGoals: Int = 12)
    -> Matrix2D
  {
    let lh = max(1e-8, lh0)
    let la = max(1e-8, la0)
    ...
    let s = m.sum
    if s > 0 { for i in 0...maxGoals { for j in 0...maxGoals { m[i, j] /= s } } }
    return m
  }
  ...
  static func totalOver(_ m: Matrix2D, _ line: Double) -> Double {
    var p = 0.0
    for i in 0..<m.n { for j in 0..<m.n { if Double(i + j) > line { p += m[i, j] } } }
    return p
  }
  ...
  static func kelly(p: Double, odds: Double, cap: Double = 0.02, haircut: Double = 0.75) -> Double {
    let b = odds - 1
    let q = 1 - p
    guard b > 0 else { return 0 }
    return min(cap, max(0, (b * p - q) / b) * 0.25 * haircut)
  }
  ...
  static func monteCarloOutcome(_ matrix: Matrix2D, n: Int = 50000, seed: UInt64 = 42) -> (
    ...
    for _ in 0..<n {
      let u = deterministicUniform(&seed)
      var idx = 0
