...
struct RootView: View {
    ...
    private var diagnosticsView:some View{List{Section("Engine"){Text(settings.engineVersion);Text("Dixon–Coles");Text("Poisson / Negative Binomial");Text("Monte Carlo");Text("Glicko adjustment");Text("Bottom-Up Player Assembly / ExpMin");Text("Referee profile");Text("Market consensus / MAD guard");Text("Sharp bookmaker guard");Text("DCS / QCS / Robust EV / Kelly");Text("Portfolio correlation");Text("Local calibration / walk-forward")};Section("Последний запуск"){ForEach(diagnostics,id:\.self){Text($0).font(.caption.monospaced())}}}.navigationTitle("Контроль")}
