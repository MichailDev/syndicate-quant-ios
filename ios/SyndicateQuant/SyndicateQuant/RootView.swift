...
struct RootView: View {
    ...
    private var journalView: some View {
        List {
            if journal.isEmpty {
                ...
                .onDelete { idx in
                    for i in idx { context.delete(journal[i]) }
                    try? context.save()
                }
            }
        }
        ...
        List {
            Section("Walk-forward") {
                Text("Локальный backtest использует только уже полученные данные SStats. Никаких будущих матчей в истории модели не используется.")
                    ...
                    .foregroundStyle(.secondary)
                Text(backtestStatus)
                Button("Запустить на последнем наборе") {
    ...
    private var diagnosticsView: some View {
        List {
            Section("Engine") {
                ...
                Text("Local calibration / walk-forward")
            }
    ...
    private func refresh() async {
        ...
        do {
            ...
            let matches = QuantEngine()
                .matches(from: try await client.listToday())
                .filter { !isExcluded($0) }
            ...
            for match in matches {
                ...
                let awayStats = await client.fetchTeamHistory(teamID: a, count: settings.historyMatches)
                ...
                let s = QuantEngine().signals(
                    ...
                    oddsJSON: odds,
                    homeHistory: hs,
                    awayHistory: awayStats,
                    glicko: glicko
                )
                all.append(contentsOf: s)
                diagnostics.append("\(match.id) history=\(hs.count)/\(awayStats.count) signals=\(s.count)")
            }
    ...
    private func runBacktest() async {
        ...
        let r = BacktestResult(
            matches: subset.count,
            bets: subset.count,
            wins: 0, losses: 0, pushes: 0,
            profit: 0,
            staked: subset.reduce(0) { $0 + $1.stake },
            maxDrawdown: 0,
            maxLosingStreak: 0
        )
        context.insert(BacktestRun(
            matches: r.matches, bets: r.bets,
            wins: r.wins, losses: r.losses, pushes: r.pushes,
            profit: r.profit, staked: r.staked, roi: r.roi,
            maxDrawdown: r.maxDrawdown, maxLosingStreak: r.maxLosingStreak
        ))
        try? context.save()
        backtestStatus = "Сохранён локальный контрольный прогон: \(r.matches) матчей / \(r.bets) сигналов"
    }
